...
import { HelpdeskMockApiService } from '@helpdesk/lib/mock-api/mock-api.service';

@Injectable({
    providedIn: 'root'
})
export class HelpdeskMockApiInterceptor implements HttpInterceptor
{
    /**
     * Constructor
     */
    constructor(
        @Inject(HELPDESK_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _helpdeskMockApiService: HelpdeskMockApiService
    )
    {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>
    {
        ...
        const {
                  handler,
                  urlParams
              } = this._helpdeskMockApiService.findHandler(request.method.toUpperCase(), request.url);
