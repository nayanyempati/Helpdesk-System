...
// -----------------------------------------------------------------------------------------------------
// @ HELPDESK TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--helpdesk-${e(key)}`, value], [`--helpdesk-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            ]))
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`helpdesk.customProps.background.${colorScheme}`);
            const foreground = theme(`helpdesk.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--helpdesk-${e(key)}`, value], [`--helpdesk-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--helpdesk-${e(key)}`, value], [`--helpdesk-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--helpdesk-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--helpdesk-on-${name}-rgb), <alpha-value>)`]
                    ])))
                },
                helpdesk  : {
                    customProps: {
